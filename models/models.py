...
class SaleOrder(models.Model):
    ...
    def _onchange_partner_id(self):
        if self.partner_id:
            ...
            sale_orders = self._fetch_sale_orders(domain, config_params['last_no_of_orders'])
            histories = self._prepare_order_histories(sale_orders, config_params['last_no_of_orders'])
    ...
    def _get_config_params(self):
        ...
        last_no_of_days = int(config_param.get_param('sale.last_no_of_days', 0))
        order_stages = config_param.get_param('sale.order_stages', 'all')
        return {
            'last_no_of_orders': last_no_of_orders,
            'last_no_of_days': last_no_of_days,
    ...
    def _build_order_domain(self, config_params):
        ...
        if config_params['order_stages'] != 'all':
            domain.append(('state', '=', config_params['order_stages']))
        else:
            domain.append(('state', 'in', ['draft', 'sent', 'sale', 'done', 'cancel']))
        if config_params['last_no_of_days']:
            today = fields.Date.today()
            start_date = fields.Date.to_string(today - timedelta(days=config_params['last_no_of_days']))
            domain.append(('date_order', '>=', start_date))
        return domain

    def _fetch_sale_orders(self, domain, limit):
        return self.env['sale.order'].search(domain, order='date_order desc', limit=limit)
